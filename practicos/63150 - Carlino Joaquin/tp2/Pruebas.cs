    ...
    class Pruebas {
        public static void Ejecutar() {
            ...
            Probar(ref numero, "Errores de parsing del enunciado", () => {
                AfirmarExcepcion<Exception>(() => Compilador.Parse("(1 + 2"), "Se esperaba ')'", "paréntesis sin cerrar");
                AfirmarExcepcion<Exception>(() => Compilador.Parse(""), "Entrada vacía", "entrada vacía");
                AfirmarExcepcion<Exception>(() => Compilador.Parse("1 + ?"), "Token inesperado", "token inesperado");
            });
            ...
            Console.WriteLine($"\nTodas las pruebas pasaron correctamente! Total: {numero - 1} grupos evaluados");
        }

        private static void Probar(ref int numero, string descripcion, Action accion) {
            Console.WriteLine($"{numero}. {descripcion} -> En proceso de ejecucion");
            accion();
